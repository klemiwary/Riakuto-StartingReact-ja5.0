...
export default function RegistrationEmailForm() {
  ...
  useEffect(() => {
    const targetForm = document.forms.namedItem(form.id);
    const preventReset = (event: Event) => {
      event.preventDefault();
    };
    targetForm?.addEventListener("reset", preventReset, true);

    return () => {
      targetForm?.removeEventListener("reset", preventReset, true);
    };
  }, [form.id]);
