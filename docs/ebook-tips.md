...
日本で販売されている Kindle 端末の最新のシステムには筑紫明朝フォントがインストールされています。オリジナルの PDF 版の読書体験に近づけるために、この筑紫明朝フォントを適用していただきたいところですが、仕様として埋め込みフォントのある AZW3 ファイルの閲覧時にリーダー側でフォントを変更すると、本文以外のすべての要素にもそのフォントが適用されてしまうようです。

ですので残念ですが、デフォルトのフォント設定のままでお読みください。
